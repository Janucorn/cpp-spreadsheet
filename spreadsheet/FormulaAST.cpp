...
// При делении на 0 выбрасывайте ошибку вычисления FormulaError
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        double result;
        const auto& lhs = lhs_->Evaluate(sheet);
        const auto& rhs = rhs_->Evaluate(sheet);
        switch (type_) {
        case Add:
            result = lhs + rhs;
            break;
...
// Реализуйте метод Evaluate() для унарных операций.
    double Evaluate(const SheetInterface& sheet) const override {
        return type_ == Type::UnaryMinus ? operand_->Evaluate(sheet) * (-1) : operand_->Evaluate(sheet);
    }
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        // если ячейка пуста, то считаем, что значение 0.0
        if (cell == nullptr) {
            return 0.0;
        }
...
public:
    ...
    void exitCell(FormulaParser::CellContext* ctx) override {
        auto valueStr = ctx->CELL()->getSymbol()->getText();
        auto value = Position::FromString(valueStr);
        if (!value.IsValid()) {
            throw FormulaException("Invalid position: " + valueStr);
        }
